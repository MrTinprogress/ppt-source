...
const opacity = computed<number>(() => +(formatter.value.glowOpacity ?? 0.26))
...
<template>
  <div>
    ...
    >
      <div
        class="clip deck-light-glow deck-light-glow--blue"
        :style="{ 'clip-path': `polygon(${poly1})`, 'opacity': opacity }"
      />
      <div
        class="clip deck-light-glow deck-light-glow--cyan"
        :style="{ 'clip-path': `polygon(${poly2})`, 'opacity': opacity }"
      />
      <div
        class="clip deck-light-glow deck-light-glow--violet"
...
.deck-light-glow--blue {
  background:
    radial-gradient(circle at 40% 38%, rgba(96, 165, 250, 0.95), transparent 58%),
    linear-gradient(120deg, rgba(186, 230, 253, 0.92), rgba(147, 197, 253, 0.62));
}

.deck-light-glow--cyan {
  background:
    radial-gradient(circle at 58% 44%, rgba(45, 212, 191, 0.86), transparent 58%),
    linear-gradient(240deg, rgba(204, 251, 241, 0.9), rgba(103, 232, 249, 0.5));
}

.deck-light-glow--violet {
  background:
    radial-gradient(circle at 50% 42%, rgba(196, 181, 253, 0.8), transparent 60%),
    linear-gradient(0deg, rgba(221, 214, 254, 0.78), rgba(191, 219, 254, 0.36));
}
